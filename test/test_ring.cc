...
TEST(Ring, CppBase)
{
	std::vector<char> buf(sizeof(tll::Ring) + 128);
	auto ring = (tll::Ring *) buf.data();
	ring->init(128);
...
{
	std::vector<char> ringbuf(sizeof(tll::Ring) + 1024 * 1024);
	auto ring = (tll::PubRing *) ringbuf.data();
	ring->init(1024 * 1024);

	const size_t count = 1000000;
	bool stop = false;
	std::thread t(cppringpub, ring, count, &stop);
