...
{
	ringbuffer_t ring;
	ring_init(&ring, 1024 * 1024, nullptr);

	auto rpp = (tll::Ring *) ring.header;
	auto pub = (tll::PubRing *) ring.header;
	pub->init(ring.header->size);
	...
	timeit(count, "c++-pub", ringpubcpp<tll::PubRing>, pub, &idx); idx = 0;
	timeit(count, "c", ringpub, &ring, &idx); idx = 0;
