...
TLL_DECLARE_IMPL(tll::channel::TimeLine);
...
tll_channel_t * tll_channel_context_t::init(const tll::Channel::Url &_url, tll_channel_t *master, const tll_channel_impl_t * impl)
{
	...
	do {
		if (impl->version > TLL_CHANNEL_IMPL_VERSION_CURRENT)
			return _log.fail(nullptr, "Impl version {} is in the future, last known is {}", impl->version, (int) TLL_CHANNEL_IMPL_VERSION_CURRENT);
		*c = {};
		c->context = this;
		c->impl = impl;
