...
template <typename T>
inline result_t<bool> hex2bin(std::string_view s, T & buf)
{
	if (s.size() % 2)
...
template <typename T>
inline std::string b64_encode(const T &s)
{
	std::string r;
	b64_encode(s, r);
	return r;
...
template <typename T>
inline result_t<bool> b64_decode(std::string_view s, T & buf)
{
	if (s.size() == 0) {
		buf.resize(0);
		return true;
...
inline result_t<std::vector<char>> b64_decode(std::string_view s)
{
	std::vector<char> buf;
