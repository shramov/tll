...
static inline bool match(const std::vector<std::string_view> &mask, const std::string_view &path)
{
	auto ps = split<'.'>(path);
	auto mi = mask.begin();
	auto pi = ps.begin();
...
static inline bool match(const std::string_view &mask, const std::string_view &path)
{
