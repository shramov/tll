...
int Control::_init(const tll::Channel::Url &url, tll::Channel * master)
{
	...
	if (auto r = url.getT("control.service-tags", std::vector<std::string>()); r) {
		if (r->size() > 0)
			_service_tags = *r;
		else if (r = url.getT("service-tags", std::vector<std::string>()); r)
			_service_tags = *r;
		else
			return _log.fail(EINVAL, "Invalid parameter service-tags: {}", r.error());
	} else
		return _log.fail(EINVAL, "Invalid parameter control.service-tags: {}", r.error());
...
int Control::_on_uplink_active(tll::Channel * c, const tll_msg_t * msg)
{
	...
	if (_service_tags.size()) {
		data.get_tags().resize(_service_tags.size());
		auto idx = 0u;
		for (auto & tag : _service_tags)
			data.get_tags()[idx++] = tag;
	}
