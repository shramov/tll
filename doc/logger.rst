...
``async`` key enables (or disables) async logging described in next section. If value is true
(``yes``) then new thread is spawned, if false (``no``) then existing thread is stopped. If there is
no such key - async settings are not changed. In async mode messages are stored in internal ring
buffer and are passed to backend in separate thread. When this mode is disabled (or application
exits and static logger context is destructed) all entries from buffer are flushed.
