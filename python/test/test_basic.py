...
def test_direct():
    ...
    with pytest.raises(TLLError): Accum('direct://', name='dup', master=c, context=ctx)
...
@pytest.mark.parametrize("name,messages", [("stream-client", ["Online", "EndOfBlock", "BeginOfBlock"]), ("tcp-client", ["WriteFull", "WriteReady", "WriteFlush"])])
